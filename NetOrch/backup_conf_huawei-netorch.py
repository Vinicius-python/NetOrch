...
def backup_switch(ip, username, password, output_file):
    try:
        ...
        device = {
            'device_type': 'huawei',  # Tipo de dispositivo Cisco IOS
            'ip': ip,
            'username': username,
            'password': password,
        ...
        net_connect.send_command('screen-length 0')
...
with open('/usr/local/scripts/routers_huawei-netorch', 'r') as routers_file:
    for line in routers_file:
        name, ip = line.strip().split(':')
        username = 'admin'
        password = 'admin'
        backup_switch(ip, username, password, f"/var/www/html/netorch/huawei/config-backup_{name}.{current_timestamp}.cfg")
