...
log_filename = f'/var/www/html/riogaleao/hp/backup_hp_log/backup_hp_{current_timestamp}.log'
...
def backup_switch(ip, username, password, output_file):
    try:
        ...
        device = {
            'device_type': 'hp_comware_ssh',  # Tipo de dispositivo Cisco IOS
            'ip': ip,
            'username': username,
            'password': password,
            #'secret': password,  # Senha secreta (normalmente a mesma senha)
            #'timeout': 90,  # Tempo limite da conexão SSH
            #'read_timeout_override': 60,  # Tempo limite da conexão SSH
        }
...
with open('/usr/local/scripts/routers_hp-netorch', 'r') as routers_file:
    for line in routers_file:
        name, ip = line.strip().split(':')
        username = 'admin'
        password = 'admin'
        backup_switch(ip, username, password, f"/var/www/html/netorch/hp/config-backup_{name}.{current_timestamp}.cfg")
